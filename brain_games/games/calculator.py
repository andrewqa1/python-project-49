...
from brain_games.utils.randoms import get_rand_number_from_range, get_rand_elem_from_seq
...
def play():
    ...
    for _ in range(3):
        ...
        if result != real_result:
            failure(
                name=name,
                additional_text=f'"{result}" is wrong answer ;(. Correct answer was "{real_result}".'
            )
            return
